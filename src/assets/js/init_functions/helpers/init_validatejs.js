...
validate.validators.require_if = function (value, options, key, attributes) {
    if (attributes[options.other_attribute_name] == options.other_attribute_value) {
        if (value == null) {
            return options.message;
        }
...
//init validator js
function init_validatejs(form, constraints, function_name, selector_loader_area) {
    ...
    for (var i = 0; i < submits.length; ++i) {
        submits.item(i).addEventListener("click", function (ev) {
            ...
            handleFormSubmit(form, constraints, function_name, submit_value, selector_loader_area);
        });
    }
...
function handleFormSubmit(form, constraints, function_name, submit_value, selector_loader_area) {
